...
public class UserVerwalten {
    ...
    public boolean speichereUser(User user) throws SQLException {
        String insertQuery = "INSERT INTO user_account (email, vorname, nachname, password_hash, role, assigned_center) VALUES (?,?,?,?,?,?)";
        try (Connection conn = dataSource.getConnection();
    ...
    public User findeUser(String email) throws SQLException {
        String query = "SELECT user_id, email, password_hash, vorname, nachname, role, geburtsdatum, assigned_center FROM user_account WHERE email = ?";
        try (Connection conn = dataSource.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, email);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    user.setAssignedCenter(rs.getString("assigned_center"));
                    return user;
                }
            }
        }
    ...
    public void weiseCenterZu(String email, String centerName) throws SQLException {
        String updateQuery = "UPDATE user_account SET assigned_center=? WHERE email=?";
        try (Connection conn = dataSource.getConnection();
