    ...
    public JSONArray getVaccinesForCenter(int centerId) throws Exception {
        ...
        try (Connection conn = DbUtils.getConnection()) {
            PreparedStatement ps = conn.prepareStatement(
                "SELECT v.id, v.name, COALESCE(cv.available_doses, 0) as doses " +
                "FROM vaccine v " +
                "LEFT JOIN vaccination_center_vaccine cv ON v.id = cv.vaccine_id AND cv.center_id = ?"
            );
    ...
    public boolean updateInventory(int centerId, int vaccineId, int dosesToAdd) throws Exception {
        try (Connection conn = DbUtils.getConnection()) {
            PreparedStatement checkPs = conn.prepareStatement(
                "SELECT available_doses FROM vaccination_center_vaccine WHERE center_id = ? AND vaccine_id = ?"
            );
            ...
            if (rs.next()) {
                ...
                PreparedStatement updatePs = conn.prepareStatement(
                    "UPDATE vaccination_center_vaccine SET available_doses = ? WHERE center_id = ? AND vaccine_id = ?"
                );
