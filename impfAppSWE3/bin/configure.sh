...
if [ "$(uname -n)" = hopper ]; then
  YOURUSER=$(whoami)
  DEPLOYPASSWORD=$(grep -i '^password' ~/.my.cnf | head -1 | cut -d '=' -f2- | tr -d ' ')
else
  ...
  DEPLOYPASSWORD=$(ssh hopper "grep -i '^password' .my.cnf | head -1 | cut -d '=' -f2- | tr -d ' '")
fi
...
echo "App-URL: https://informatik.hs-bremerhaven.de/docker-$YOURUSER-java/"
echo "Deploy-Manager: https://informatik.hs-bremerhaven.de/docker-$YOURUSER-manager/"
