...
public class BookingService {
    ...
    try (Connection connection = DbUtils.getConnection()) {
      PreparedStatement ps =
          connection.prepareStatement(
              "SELECT b.id, b.status, p.first_name, p.last_name, t.start_time, t.end_time, v.name"
                  + " AS vaccine_name, vc.name AS center_name, vc.address AS center_address FROM"
                  + " booking b JOIN person p ON b.person_id = p.id JOIN timeslot t ON"
                  + " b.timeslot_id = t.id JOIN vaccine v ON b.vaccine_id = v.id JOIN"
                  + " vaccination_center vc ON t.center_id = vc.id WHERE b.account_id = ? ORDER BY"
                  + " t.start_time");
      ps.setInt(1, userId);
    ...
    try (Connection connection = DbUtils.getConnection()) {
      ...
      try {
        ...
        ConfirmationData confirmationData =
            loadConfirmationData(connection, timeslotId, vaccineId, personId, userId);
  ...
  private void fail(Map<String, Object> result, Connection connection, String message)
      throws SQLException {
  ...
  private String checkVaccineAvailability(Connection connection, int timeslotId, int vaccineId)
      throws SQLException {
    PreparedStatement ps =
        connection.prepareStatement(
  ...
  private void decrementVaccineDose(Connection connection, int timeslotId, int vaccineId)
      throws SQLException {
    PreparedStatement ps =
        connection.prepareStatement(
  ...
  private int createBooking(
      Connection connection, int personId, int userId, int timeslotId, int vaccineId)
      throws SQLException {
    PreparedStatement ps =
        connection.prepareStatement(
            "INSERT INTO booking (person_id, account_id, timeslot_id, vaccine_id, booking_date,"
                + " status) VALUES (?, ?, ?, ?, NOW(), 'CONFIRMED')",
  ...
  private ConfirmationData loadConfirmationData(
      Connection connection, int timeslotId, int vaccineId, int personId, int accountId)
      throws SQLException {
    PreparedStatement ps =
        connection.prepareStatement(
  ...
  private void sendConfirmationBestEffort(
    ...
    try {
      ...
      // Buchung war bereits erfolgreich; E-Mail/PDF ist "best effort".
    }
  }
  ...
  /** Gibt die Details eines einzelnen Termins zurück */
      ...
      PreparedStatement ps =
          connection.prepareStatement(
              "SELECT b.id, b.status, p.first_name, p.last_name, t.start_time, t.end_time, v.name"
                  ...
                  + " booking b JOIN person p ON b.person_id = p.id JOIN timeslot t ON"
                  + " b.timeslot_id = t.id JOIN vaccine v ON b.vaccine_id = v.id JOIN"
                  + " vaccination_center vc ON t.center_id = vc.id WHERE b.id = ?");
  ...
  public boolean updateAppointmentStatus(int bookingId, String newStatus) throws Exception {
    try (Connection connection = DbUtils.getConnection()) {
      // Prüfen, ob der Termin existiert und nicht storniert ist
      PreparedStatement checkPs =
          connection.prepareStatement("SELECT status FROM booking WHERE id = ?");
      ...
      if (rs.next()) {
        ...
        if ("CANCELLED".equals(currentStatus)) {
          // Stornierte Termine können nicht aktualisiert werden
          return false;
        }

        // Status aktualisieren
        PreparedStatement updatePs =
            connection.prepareStatement("UPDATE booking SET status = ? WHERE id = ?");
  ...
  /**
   * Storniert einen Termin des eingeloggten Nutzers. Setzt booking.status auf CANCELLED und bucht
   * die Impfstoffdosis zurück.
   */
  public Map<String, Object> cancelAppointment(int userId, int bookingId) throws Exception {
    ...
    try (Connection connection = DbUtils.getConnection()) {
      connection.setAutoCommit(false);
      try {
        // Buchung holen und sperren (damit keine Race-Conditions entstehen)
        PreparedStatement ps =
            connection.prepareStatement(
        ...
        PreparedStatement cancelPs =
            connection.prepareStatement(
                "UPDATE booking SET status = 'CANCELLED' WHERE id = ? AND account_id = ? AND status"
                    + " = 'CONFIRMED'");
