...
public class DoseService {
    public void addDoses(String vaccine, int quantity, String centerName) throws SQLException {
        try (Connection conn = Database.getConnection()) {
            ...
            if (centerId == -1) {
                throw new SQLException("Impfzentrum nicht gefunden.");
            }
    ...
    public int getStockForVaccine(String centerName, String vaccine) throws SQLException {
        try (Connection conn = Database.getConnection()) {
            String q = "SELECT vi.stock FROM vaccine_inventory vi JOIN vaccine_center vc ON vi.center_id = vc.center_id WHERE vc.center_name=? AND vi.vaccine=?";
    ...
    public ResultSet getAllStocksForCenter(String centerName) throws SQLException {
        Connection conn = Database.getConnection();
        String q = "SELECT vi.vaccine, vi.stock FROM vaccine_inventory vi JOIN vaccine_center vc ON vi.center_id=vc.center_id WHERE vc.center_name=?";
        PreparedStatement ps = conn.prepareStatement(q);
        ps.setString(1, centerName);
