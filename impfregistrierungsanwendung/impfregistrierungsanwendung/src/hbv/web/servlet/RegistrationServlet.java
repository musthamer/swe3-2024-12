...
public class RegistrationServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    String header =
        readTemplate("header.html", req).replace("<!-- Title placeholder -->", "Registrierung");
    ...
    try {
      boolean success =
          userService.registerUser(
              email, password, vorname, nachname, role, (regcode == null ? "" : regcode));
      if (success) {
        out.println(header);
        ...
        out.println("<h2>Registrierung fehlgeschlagen</h2>");
        out.println(
            "<p>Entweder ist die E-Mail bereits registriert oder der Registrierungscode war"
                + " falsch.</p>");
        out.println("<p><a href='register'>Zurück</a></p>");
        out.println(footer);
      }
