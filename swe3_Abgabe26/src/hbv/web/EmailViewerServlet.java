...
public class EmailViewerServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    out.println(
        "<style>body{font-family:Arial,sans-serif;margin:20px} .email{border:1px solid"
            + " #ccc;padding:10px;margin-bottom:20px;} pre{background:#f5f5f5;padding:10px;}"
            + " .attachment{margin-top:10px;padding:5px;background-color:#e0f7fa;}</style>");
            ...
            try {
              ...
              if (emailData.has("attachment")) {
                ...
                if (attachment.has("filename") && attachment.has("content")) {
                  ...
                  out.println(
                      "<p><strong>Anhang:</strong> " + attachment.optString("filename") + "</p>");
                  out.println(
                      "<p><a href='?pdf="
                          + emailIndex
                          + "' target='_blank'>Anhang anzeigen/herunterladen</a></p>");
                  out.println("</div>");
                }
              }
            } catch (Exception e) {
              // Fallback: Zeige den rohen JSON-Text
  ...
  /** Zeigt einen PDF-Anhang direkt an */
  private void showPdfAttachment(
      HttpServletRequest request, HttpServletResponse response, String pdfIdStr)
      throws IOException, ServletException {
