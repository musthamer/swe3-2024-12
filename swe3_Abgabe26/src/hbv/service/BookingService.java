...
public class BookingService {
    ...
    public Map<String, Object> bookAppointment(int userId, int timeslotId, int vaccineId, int personId,
            String baseUrl, String webapp) throws Exception {
    ...
    private void sendConfirmationBestEffort(int bookingId, ConfirmationData data, String baseUrl, String webapp) {
        ...
        try {
            ...
            EmailService.send(EmailMessageFactory.bookingConfirmation(
    ...
    private int getCenterIdForTimeslot(int timeslotId) throws Exception {
        try (Connection connection = DbUtils.getConnection();
             PreparedStatement ps = connection.prepareStatement(
                 "SELECT center_id FROM timeslot WHERE id = ?"
             )) {
            ps.setInt(1, timeslotId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    /**
     * Storniert einen Termin des eingeloggten Nutzers.
     * Setzt booking.status auf CANCELLED und bucht die Impfstoffdosis zurück.
     */
