...
public class DoseService {
  public void addDoses(String vaccine, int quantity, String centerName) throws SQLException {
    try (Connection conn = Database.getConnection()) {
      int centerId = -1;
      PreparedStatement psCenter =
          conn.prepareStatement("SELECT center_id FROM vaccine_center WHERE center_name = ?");
      ...
      PreparedStatement psCheck =
          conn.prepareStatement(
              "SELECT stock FROM vaccine_inventory WHERE center_id = ? AND vaccine = ?");
      ...
      if (exists) {
        ...
        PreparedStatement psUpdate =
            conn.prepareStatement(
                "UPDATE vaccine_inventory SET stock = ? WHERE center_id = ? AND vaccine = ?");
        ...
        PreparedStatement psInsert =
            conn.prepareStatement(
                "INSERT INTO vaccine_inventory (center_id, vaccine, stock) VALUES (?,?,?)");
