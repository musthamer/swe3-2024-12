...
public class PersonService {
    ...
    /**
     * Ermittelt oder legt die Person für eine Buchung an.
     * forSelf=true: Account-Inhaber; forSelf=false: Drittperson (Familienmitglied).
     */
    public int resolvePersonForBooking(int userId, String firstName, String lastName,
            String dateOfBirth, boolean forSelf) throws Exception {
        try (Connection connection = DbUtils.getConnection()) {
            ...
            try {
                ...
                if (forSelf) {
                    ...
                    if (personId <= 0) {
                        personId = createAccountHolderPerson(connection, userId, firstName, lastName, dateOfBirth);
                    } else {
    ...
    private int findDependent(Connection connection, int userId, String firstName,
            String lastName, String dateOfBirth) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement(
                "SELECT id FROM person " +
                "WHERE account_id = ? AND first_name = ? AND last_name = ? AND date_of_birth = ?"
        )) {
            ps.setInt(1, userId);
    ...
    private void updatePerson(Connection connection, int personId, String firstName,
            String lastName, String dateOfBirth) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement(
    ...
    private int createAccountHolderPerson(Connection connection, int userId, String firstName,
            String lastName, String dateOfBirth) throws SQLException {
        ...
        try (PreparedStatement ps = connection.prepareStatement(
            ...
            try (PreparedStatement linkPs = connection.prepareStatement(
                    "UPDATE account SET person_id = ? WHERE id = ?"
            )) {
    ...
    private int createDependent(Connection connection, int userId, String firstName,
            String lastName, String dateOfBirth) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement(
