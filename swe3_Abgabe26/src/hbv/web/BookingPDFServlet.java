    ...
    try {
      ...
      try (Connection connection = ds.getConnection()) {
        ...
        PreparedStatement ps = connection.prepareStatement(
            "SELECT b.id, p.first_name, p.last_name, t.start_time, c.name AS center_name, v.name AS vaccine_name " +
            "FROM booking b " +
            "JOIN person p ON b.person_id = p.id " +
            "JOIN timeslot t ON b.timeslot_id = t.id " +
            "JOIN vaccination_center c ON t.center_id = c.id " +
        ...
        if (rs.next()) {
          ...
          byte[] pdfData = PDFGenerator.generateVaccinationConfirmation(
              name, appointmentDate, vaccinationCenter, vaccineType, bookingId,
              ctx.getInitParameter("baseurl"), ctx.getInitParameter("webapp"));
          ...
          out.println("<!DOCTYPE html><html><body>");
          out.println("<h2>Fehler</h2>");
          out.println("<p>Die Buchung konnte nicht gefunden werden oder gehört nicht zu Ihrem Account oder wurde storniert.</p>");
          out.println("<p><a href='appointments'>Zurück zu den Terminen</a></p>");
          out.println("</body></html>");
        }
      }
